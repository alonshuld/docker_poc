...
class ImageManager:
    ...
    def build_image(self, name: str, dockerfile_path: str, dependency_dir: str = ""):
        """
        Builds a new image
        ...
        """
        if dependency_dir == "":    # If no special dependency directory sets to default
            dependency_dir = DEFAULT_DOCKER_PATH + name + "/"

        image = Image(name=name, dependency_dir=dependency_dir)
    ...
    def _file_content_to_image(self, file_content: bytes, file_name: str, dependency_dir: str = "") -> Image:
        """
        ...
        if dependency_dir == "":    # Default dir
            dependency_dir = DEFAULT_DOCKER_PATH + image_name + "/"
